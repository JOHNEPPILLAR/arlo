...
const ARLO_WEB = 'https://my.arlo.com/hmsweb';
...
const AUTH_URLS = {
  ...
  START_NEW_SESSION: `${ARLO_WEB}/users/session/v2`,
};
...
async function _getFactors() {
  debug('Get factors');
  const url = AUTH_URLS.GET_FACTORS + this.authenticated;
...
function _getArloEmails(mailServer) {
  return new Promise((resolve, reject) => {
    ...
    const waitForEmail = 10000; // 10 seconds
...
async function _verifyAuthToken() {
  ...
  try {
    const url = AUTH_URLS.VERIFY_AUTH;
    const response = await this._get(url, {});
    if (response instanceof Error || typeof response === 'undefined') {
      debug(response);
      return false;
