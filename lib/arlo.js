...
class Arlo extends EventEmitter {
  ...
  async setPrivacyActive(deviceID, privacy) {
    ...
    try {
      this.cameras[deviceIndex].properties.privacyActive = privacy;
    } catch (err) {
      debug(err);
    }
    return true;
  ...
  async isPrivacyEnabled(deviceID) {
    try {
      ...
      // If app just launched, suggest cam is in privacy mode
      if (typeof device.properties.privacyActive === 'undefined') {
        debug(`[${deviceID}] No properties so suggest privacy mode active`);
        return true;
      }
