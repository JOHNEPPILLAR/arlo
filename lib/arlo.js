...
class Arlo extends EventEmitter {
  ...
  async login() {
    ...
    // Logout and back in every 24 hrs
  ...
  async startStream(deviceID) {
    try {
      ...
      if (device.properties.streamActive) return device.properties.streamURL;
      ...
      this.cameras[deviceIndex].properties.streamURL = rtnURL;
      this.cameras[deviceIndex].properties.streamActive = true;
  ...
  async getMediaLibrarySummary(from) {
    try {
      ...
      if (!response.success) {
        const err = new Error('Error getting media library summary data');
        debug(response.message);
        return err;
      }
  ...
  async _logOut() {
    ...
    if (!response.success) debug(response.data);
  ...
  async _processEventMessage(eventData) {
    try {
      ...
      if (eventData.resource === EVENT_MEDIA_UPLOAD) {
        ...
        if (this.cameras[deviceIndex].properties.streamActive) {
          this.cameras[deviceIndex].properties.streamActive = false;
          this.cameras[deviceIndex].properties.streamURL = '';
        }
