...
class Arlo extends EventEmitter {
  ...
  async setPrivacyActive(deviceID, privacy) {
    ...
    if (!privacy) {
      debug(`[${deviceID}] Requesting new FF snapshot`);
      setTimeout(() => {
        this.getNewSnapshot.call(this, deviceID, true);
      }, 8000);
    }
   ...
   */
  async startStream(deviceID, privacyOutdated = false) {
    try {
      const deviceIndex = this.cameras.findIndex(
        (d) => d.deviceId === deviceID,
      );
      ...
      // Can not start stream if cam is off
      if (privacyOutdated)
        debug(`[${deviceID}] Cached camera privacy setting not up to date`);
      else if (
        device.properties.privacyActive ||
        typeof device.properties.privacyActive === 'undefined'
      ) {
        debug(`[${deviceID}] Camera is off, unable to request stream`);
        return false;
      }
   ...
   * Take new snapshot
   */
  async getNewSnapshot(deviceID, privacyOutdated = false) {
    try {
      ...
      // Can not request new snap shot if cam is off
      if (privacyOutdated)
        debug(`[${deviceID}] Cached camera privacy setting not up to date`);
      else if (
        device.properties.privacyActive ||
      ...
      // Start stream, otherwise ff snapshot will not work
      await this.startStream.call(this, deviceID, privacyOutdated);
      setTimeout(() => {
        debug(`[${deviceID}] Requesting snapshot`);
        this._notify(
          {
            action: 'set',
            resource: `cameras/${deviceID}`,
      ...
      if (device.deviceType === TYPE_BASESTATION) {
        ...
        for (const camera of this.cameras) {
          if (camera.deviceType === TYPE_CAMERA) {
            ...
            await this.getNewSnapshot.call(this, camera.deviceId);
          }
        }
      }
    } catch (err) {
