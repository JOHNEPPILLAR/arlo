...
const EVENT_RATLS_READY = 'ratls_open';
...
class Arlo extends EventEmitter {
  ...
  async login() {
    ...
    let proceed = await this._getHmswebVersion.call(this);
    ...
    //
    // App seems to not need mfa sign in!
    //
   ...
   */
  async downloadLocalMediaFile(ip, port, path) {
    const url = `https://${ip}:${port}/hmsls/download/${path}`;

    // Get certs
    const httpsAgent = new https.Agent({
      ca: fs.readFileSync('./certs/ica.crt'),
      cert: fs.readFileSync('./certs/peer.crt'),
      key: fs.readFileSync('./certs/private.pem'),
    });

    // Set headers
    const ratlsHeaders = this.headers;
    ratlsHeaders.authorization = `Bearer ${this.ratlsToken}`;
    ratlsHeaders.accept = `application/json`;

    const options = {
      method: 'GET',
      url,
      httpsAgent,
      headers: ratlsHeaders,
    };

    try {
      // Call local storage account to get recordings
      debug('Getting recording data');
      ...
      if (!response || !response.data.success)
        throw new Error('Error getting local media library summary data');

      debug(`Found ${response.data.data.length || 0} recordings`);
      return response.data || [];
    } catch (err) {
      debugComm(err.message);
      const returnError = new Error('Error getting local media library data');
      return returnError;
    }
  }
  ...
  async getLocalMediaLibrary(ip, port, from) {
    ...
    if (!fs.existsSync(peerCrtPath)) {
      ...
      if (fs.existsSync(publicPemPath)) {
        ...
        this.RATLSPublicKey = fs.readFileSync('./certs/public.pem', {
          encoding: 'utf8',
        });
        this.RATLSPrivateKey = fs.readFileSync('./certs/private.pem', {
          encoding: 'utf8',
        });
      } else {
        try {
          ...
          // PEM serialize
          this.RATLSPrivateKey = forge.pki.privateKeyToPem(keys.privateKey);
          this.RATLSPublicKey = forge.pki.publicKeyToPem(keys.publicKey);

          // Save certs
          fs.writeFileSync('./certs/private.pem', this.RATLSPrivateKey);
          fs.writeFileSync('./certs/public.pem', this.RATLSPublicKey);
        } catch (err) {
      ...
      let publicKey = this.RATLSPublicKey.replace(/(\r\n|\n|\r)/gm, '');
      ...
      this.RATLSPeerCert = this._formatToPem(
        response.data.certsData[0].peerCert,
      );
      fs.writeFileSync('./certs/peer.crt', this.RATLSPeerCert);

      this.RATLSDeviceCert = this._formatToPem(
        response.data.certsData[0].deviceCert,
      );
      fs.writeFileSync('./certs/device.crt', this.RATLSDeviceCert);

      this.RATLSIcaCert = this._formatToPem(response.data.icaCert);
      fs.writeFileSync('./certs/ica.crt', this.RATLSIcaCert);

      this.RATLSCombined = this.RATLSPeerCert + '\n' + this.RATLSIcaCert;
      fs.writeFileSync('./certs/combined.crt', this.RATLSCombined);
    } else {
      this.RATLSPrivateKey = fs.readFileSync('./certs/private.pem', {
        encoding: 'utf8',
      });
      this.RATLSIcaCert = fs.readFileSync('./certs/ica.crt', {
        encoding: 'utf8',
      });
      this.RATLSPeerCert = fs.readFileSync('./certs/peer.crt', {
        encoding: 'utf8',
      });
    }
    ...
    const url = `https://${ip}:${port}/hmsls/list/${dateFrom}/${dateTo}`;

    // Bind certs to http agent
    const httpsAgent = new https.Agent({
      ca: this.RATLSIcaCert,
      cert: this.RATLSPeerCert,
      key: this.RATLSPrivateKey,
    });

    // Set headers
    const headers = {
      authorization: `Bearer ${this.RATLSToken}`,
      accept: 'application/json',
  ...
  async openLocalMediaLibrary() {
    try {
      ...
      this.RATLSToken = response.data.ratlsToken;
      ...
      this._notify(body, this.baseStation);
    ...
    try {
      ...
      if (eventData.resource === EVENT_RATLS) {
        ...
        this.emit(EVENT_RATLS, {
          ip: eventData.properties.privateIP,
          port: eventData.properties.port,
        });
      }
