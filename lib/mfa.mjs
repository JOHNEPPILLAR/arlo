...
const AUTH_URLS = {
  ...
  START_NEW_SESSION: `https://${API_DOMAIN}/hmsweb/users/session/v3`,
};
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      mailServer.search(searchCriteria, (err, uids) => {
        ...
        mailServer.setFlags(uids, ['\\Seen'], (Error) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    function fetchOnMessage(message) {
      ...
      message.on('body', (stream) => {
        emailParser.simpleParser(stream, (err, mail) => {
          if (err) {
            debug(err);
            reject(err);
          }
  ...
  return new Promise(async (resolve) => {
    try {
      ...
      mailServer.once('ready', () => {
        ...
        debug('Open inbox');
        mailServer.openBox('INBOX', false, async (Error) => {
          try {
...
async function _newSession() {
  ...
  try {
    ...
    if (response instanceof Error || typeof response === 'undefined') {
      if (response.message === '1024') {
        debug('Session has expired, re-login');
        return false;
      }
      debug(response);
      return response;
