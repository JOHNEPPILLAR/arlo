...
class Arlo extends EventEmitter {
  ...
  constructor(options) {
    ...
    if (typeof options.emailUser === 'undefined')
      return Error('No Arlo email param');
    if (typeof options.emailPassword === 'undefined')
      return Error('No Arlo email password param');
    if (typeof options.emailServer === 'undefined')
      return Error('No Arlo email server param');
  ...
  async login() {
    ...
    if (!proceed) { // Token expired
      this.headers.authorization = null; // Clear out old token;
  
      proceed = await mfa._getAuthToken.call(this, this.config.arloUser, this.config.arloPassword);
    ...
    setTimeout(() => _logOut.call(this), this.tokenExpires);
  ...
  async _processEventMessage(eventData) {
    try {
      ...
      if (eventData.resource === EVENT_MODE) {
        ...
        this.logger.info(
          `Base Station is ${!this.baseStation.armed ? 'dis' : ''
          }armed - Cam motion recording is ${this.baseStation.armed ? '' : 'not '}active`,
        );
